...
export function AppConnectionDialog({
  open,
  onOpenChange,
  app,
  action,
  ...
  const handleVerifyAndSave = async () => {
    ...
    try {
      ...
      const response = await clerkFetch("/api/v1/base/login", {
        method: "POST",
      ...
      if (!response.ok) {
        ...
        throw new Error(
          errorData.error || `验证失败 (HTTP ${response.status})`
        )
      }
      if (response.ok) {
      ...
      setError(
        err instanceof Error ? err.message : "验证并保存账号失败"
      )
    } finally {
      setLoading(false)
    }
      ...
      <DialogContent>
        <DialogHeader>
          <DialogTitle>
            连接{" "}
            {app?.source_type ||
              SUPPORTED_PLATFORMS.find((p) => p.platform === selectedPlatform)
                ?.label ||
              "账号"}
          </DialogTitle>
          <DialogDescription>
        ...
        <div className="grid gap-4 py-4">
          ...
          <div className="flex items-center space-x-2">
            <Checkbox
              ...
              disabled={loading || success}
            />
            <Label htmlFor="master" className="text-sm font-medium leading-none">
              设置为主数据源
            </Label>
          </div>
          <div className="bg-amber-50 dark:bg-amber-950/20 p-3 rounded-lg text-[13px] leading-relaxed text-amber-800 dark:text-amber-200 space-y-2 border border-amber-200 dark:border-amber-800">
            ...
            <p>
              为实现数据自动同步，服务端需要获取您的账号和密码。受限于品牌登录机制，使用本工具时请勿在其他终端同时登录，否则将导致凭证失效。
              <br />
              我们会尽力保护您的信息安全，但请您知晓并自行承担潜在的安全风险。
            </p>
          </div>
          <div className="flex items-center space-x-2">
            <Checkbox
